                             ...
                             struct ieee80211_tx_rate_control *txrc)
{
    ...
    if (rc_rate == NULL) {
        if (conf_is_ht(&sc->hw->conf) &&
            (SSV_STA_HT_CAP(sta).cap & IEEE80211_HT_CAP_LDPC_CODING))
            tx_info->flags |= IEEE80211_TX_CTL_LDPC;
        if (conf_is_ht(&sc->hw->conf) &&
            (SSV_STA_HT_CAP(sta).cap & IEEE80211_HT_CAP_TX_STBC))
                                        ...
                                        struct ieee80211_sta *sta, void *priv_sta)
{
    ...
    } else if (SSV_STA_HT_CAP(sta).ht_supported == true) {
        printk("[RC init ]HT support wsid\n");
        for (i = 0; i < SSV_HT_RATE_MAX; i++) {
            if (SSV_STA_HT_CAP(sta).mcs.rx_mask[i/MCS_GROUP_RATES] & (1<<(i%MCS_GROUP_RATES)))
                ht_supp_rates |= BIT(i);
        }
        rc_sta->ht_supp_rates = ht_supp_rates;
        if (SSV_STA_HT_CAP(sta).cap & IEEE80211_HT_CAP_GRN_FLD) {
            rc_sta->rc_type = RC_TYPE_HT_GF;
            rc_sta->ht_rc_type = RC_TYPE_HT_GF;
        } else if (SSV_STA_HT_CAP(sta).cap & IEEE80211_HT_CAP_SGI_20) {
    ...
    if((rc_sta->rc_type != RC_TYPE_B_ONLY) && (rc_sta->rc_type != RC_TYPE_LEGACY_GB)) {
        if ((SSV_STA_HT_CAP(sta).ht_supported) && (sh->cfg.hw_caps & SSV6200_HW_CAP_AMPDU_TX)) {
            rc_sta->is_ht = 1;
...
static void *ssv6xxx_rate_alloc(SSV_RATE_ALLOC_ARGS)
{
